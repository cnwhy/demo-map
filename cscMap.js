...
cscMap.Public.MapHtml = 
	'<!--move_box地图移动的层-->' +
	'<div id="{thisID}move_box" class="move_box">' +
		'<!--map_bg地图背景层-->' +
		'<div id="{thisID}map_bg" class="map_bg">' +
			'<!--map_bg_imgs背景-->' +
			'<div id="{thisID}map_bg_imgs" class="map_bg_imgs"></div>' +
			'<!--map_marks背景-->' +
			'<div id="{thisID}map_bg_move" style="width:100%; height:100%; position:absolute; left:0; top:0;"></div>' +
			'<div id="{thisID}map_bg_marks" class=""></div>' + 
		'</div>' + 
		'<!--用户层，实时插入覆盖物-->' + 
		'<div id="{thisID}user_data"></div>' + 
	'</div>' + 
	
	'<!--操作控件层,独立于地图存在,主要是调用地图API操作地图-->' +
	'<div id="{thisID}user_comm"></div>'+
	'<div class="point"></div>';//标记中心点位置，后期删除；
/*
